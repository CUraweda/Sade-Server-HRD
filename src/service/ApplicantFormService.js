...
class ApplicantFormService {
  ...
  createInterview = async (body, id, condition, employee) => {
    ...
    if (condition === "lulus") {
      ...
      const applicantInterviewData = await this.applicantInterviewDao.create({
        ...body,
        form_id: id,
      ...
      if (!applicantInterviewData)
        return responseHandler.returnError(
          httpStatus.BAD_REQUEST,
          "Gagal dalam membuat Applicant Interview"
        );
      console.log(applicantInterviewData);
    }
  ...
  aggregateToEmployee = async (id, addedPayload) => {
    ...
    const payload = {
      ...
      still_in_probation: true,
    };
    const employeeData = await this.employeeDao.create(payload);
    if (!employeeData)
      return responseHandler.returnError(
        httpStatus.BAD_REQUEST,
        "Gagal membuat data employee"
      );
    const updatedApplicant = await this.applicantFormDao.updateById(
